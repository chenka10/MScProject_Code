...
def get_encoded_frames_gestures(frame_encoder,frames,gestures,batch_size,seq_len):
  ...
  # encode all frames
  encoded_frames,encoded_frames_mu, encoded_frames_logvar, encoder_skips = frame_encoder(frames_batched)
  encoded_frames = encoded_frames.view(batch_size,seq_len,encoded_frames.size(-1))
  encoded_frames_mu = encoded_frames_mu.view(batch_size,seq_len,encoded_frames.size(-1))
  encoded_frames_logvar = encoded_frames_logvar.view(batch_size,seq_len,encoded_frames.size(-1))
  ...
  return frames_gestures_past, frames_gestures_future, encoder_skips, encoded_frames_mu, encoded_frames_logvar
...
for epoch in range(num_epochs):
    ...
    for batch in tqdm(dataloader_valid):        
        ...
        distance_weight = get_distance(positions[:,:-1,:],positions[:,1:,:]).sum(dim=1)/seq_len
        batch_mover = distance_weight.argmax().item()
        batch_least_mover = distance_weight.argmin().item()
