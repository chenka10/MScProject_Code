...
from Code.splatCoords import generate_blobs_image
...
def train(models, blob_position_encoders, dataloader_train, optimizer, params, config, device):

  loss_fn_vgg.to(device)

  frame_encoder, frame_decoder, generation_lstm = models
  ...
  for batch in tqdm(dataloader_train):
    ...
    for t in range(1,params['seq_len']):
      ...
      #   conditioning_vec = positions[:,t,:]

      num_blobs = len(blob_position_encoders)
      blobs = torch.zeros([batch_size,num_blobs,5]).to(device)
      blobs[:,0,:] = blob_position_encoders[0](positions)


      blobs_images = generate_blobs_image()
