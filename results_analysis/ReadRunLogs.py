...
for subject_i,subject in enumerate(subjects):
    ...
    for i,runid in enumerate(runids):
        ...
        if i==0:
            plt.plot(last_epoch_ssim_valid.values, marker = 'x', label=f'{i+1} (epochs: {num_epochs})',alpha=0.2,color='red')
        else:
            plt.plot(last_epoch_ssim_valid.values, marker = 'x', label=f'{i+1} (epochs: {num_epochs})',alpha=0.2,color='blue')
...
fig.savefig(f'new_SSIM_results_all_position_blob_svg_1.png')
plt.close()
