...
class Encoder128(nn.Module):
    ...
    def forward(self, input):
        ...
        x = h6.view(-1, self.dim)

        if was_batch_seq:
          h1 = self.batch_to_batch_seq(h1,128)
          h2 = self.batch_to_batch_seq(h2,64)
          h3 = self.batch_to_batch_seq(h3,32)
          h4 = self.batch_to_batch_seq(h4,16)
          h5 = self.batch_to_batch_seq(h5,8)
          h6 = self.batch_to_batch_seq(h6,4)
